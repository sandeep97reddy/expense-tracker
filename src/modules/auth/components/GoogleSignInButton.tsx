/**
 * GoogleSignInButton
 * A branded button for Google OAuth sign in.
 */
...
export function GoogleSignInButton({ onPress, loading, disabled }: GoogleSignInButtonProps) {
  ...
  // Standard Google button styles depending on theme
  const bgColor = isDark ? '#FFFFFF' : '#FFFFFF';
  const textColor = '#1F2937'; // Dark gray text on white background
    ...
    >
      {loading ? (
        ...
        <>
          ...
          <Text style={[styles.text, { color: textColor }]}>Sign in with Google</Text>
        </>
      )}
    </TouchableOpacity>
  );
